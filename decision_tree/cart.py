...
def chooseBestFeature(dataSet):
    ...
    for i in range(featureCount):
        ...
        for value in uniqueValOfFeature:
            ...
            GiniIndex += divide_prob * calc_Gini(subDataSet)
        print("当前划分属性基尼指数:", GiniIndex, "当前划分依据:", labels[i])
        # 将当前得到的基尼指数与最佳基尼指数进行对比，进行更新
        if GiniIndex < bestGiniGain:
            bestGiniGain = GiniIndex
            bestFeature = i
    return bestFeature
